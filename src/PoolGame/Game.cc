...
namespace Game
{
	...
	{
		// Constants
		static constexpr double GameObjectScale = 2.0;
	...
	void SortAndSweep(GameData & gameData, const Utilities::TaskManager::JobContext &context)
	{
		...
		auto jobB = Utilities::TaskManager::CreateLambdaBatchedJob(
			[&gameData, &createdGroups](int i, const Utilities::TaskManager::JobContext& context)
			{
				if (gameData.extremes[i].min)
					for (int j = i + 1; j < GameData::ExtremesSize && gameData.extremes[i].index != gameData.extremes[j].index; ++j)
						...
						{
							if (HasCollision(gameData.gameObjects, gameData.extremes[i].index, gameData.extremes[j].index))
							{
								const GameData::ContactData contact{ GenerateContactData(gameData.gameObjects, gameData.extremes[i].index, gameData.extremes[j].index) };
								...
								if (it == createdGroups[context.threadIndex].end())
								{
									...
									if (it == createdGroups[context.threadIndex].end())
									{
										...
										{
											gameData.contactGroups[context.threadIndex][gameData.contactGroupsSizes[context.threadIndex]++] = GameData::ContactGroup{
										...
										{
											gameData.contactGroups[context.threadIndex].push_back(GameData::ContactGroup{
												{ contact.a , contact.b }, // game object indexes
												{ contact } // ContactData
											}); // creem la llista de colisions nova
										}

										createdGroups[context.threadIndex][contact.a] = &gameData.contactGroups[context.threadIndex]
											[gameData.contactGroups[context.threadIndex].size() - 1]; // guardem referència a aquesta llista
										createdGroups[context.threadIndex][contact.b] = &gameData.contactGroups[context.threadIndex]
											[gameData.contactGroups[context.threadIndex].size() - 1]; // per cada objecte per trobarla
									}
									else
									{
										it->second->contacts.push_back(contact); // afegim la colisió a la llista
										it->second->objectIndexes.push_back(contact.a);
								...
								else
								{
									...
									{
										groupA->objectIndexes.push_back(contact.b);
			...
			"Sweep + Fine-Grained",
			20'000/*GameData::ExtremesSize / (GameData::ExtremesSize < Utilities::Profiler::MaxNumThreads ? 1 : Utilities::Profiler::MaxNumThreads)*/,
			GameData::ExtremesSize
		);
	...
	void SolveCollisionGroups(GameData & gameData, const Utilities::TaskManager::JobContext &context)
	{
		...
		for (int i = 0; i < Utilities::Profiler::MaxNumThreads; ++i)
			if (!gameData.contactGroups[i].empty())
		...
		if (mergedContactGroups.size())
		{
				...
				[&gameData, &mergedContactGroups](int i, const Utilities::TaskManager::JobContext& context)
			{
				...
				"Group Solver",
				5'000/*mergedContactGroups.size() / (mergedContactGroups.size() < Utilities::Profiler::MaxNumThreads ? 1 : Utilities::Profiler::MaxNumThreads)*/,
	...
	RenderData Update(GameData & gameData, const InputData& inputData, const Utilities::TaskManager::JobContext &context)
	{
		...
		auto job = Utilities::TaskManager::CreateLambdaBatchedJob(
			...
			"Fill Render Data",
			20'000/*GameData::MaxGameObjects / (GameData::MaxGameObjects < Utilities::Profiler::MaxNumThreads ? 1 : Utilities::Profiler::MaxNumThreads)*/,
			GameData::MaxGameObjects);
		context.DoAndWait(&job);
